...
puts "Creating project - please be patient - this may take a minute or two..."
...
sbp_config_ip -vlnv {latticesemi.com:ip:gpio0:1.6.2}  -meta_vlnv {latticesemi.com:ip:gpio:1.6.2} -cfg_value {IO_LINES_COUNT:8,EXTERNAL_BUF:true,DIRECTION_DEF_VAL_INPUT:FF}
sbp_config_ip -vlnv {latticesemi.com:ip:i2c_ctrl:2.0.1}  -meta_vlnv {latticesemi.com:ip:i2c_controller:2.0.1} -cfg_value {SYS_CLOCK_FREQ:60}
...
sbp_add_component -vlnv {latticesemi.com:ip:gpio0:1.6.2} -name gpio0_inst
sbp_add_component -vlnv {latticesemi.com:ip:i2c_ctrl:2.0.1} -name i2c_ctrl_inst
...
sbp_add_port -from 7 -to 0 -direction in gpio_i

puts "Connecting nets..."
...
sbp_connect_net "$projectName/gpio0_inst/gpio_i" "$projectName/gpio_i" 

puts "Connecting interface nets..."
...
puts "Saving and generating design..."
sbp_design save
sbp_design generate

puts "Right-click in Schematic pane and click Relayout menu item."
